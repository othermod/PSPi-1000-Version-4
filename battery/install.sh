...
grep battery /etc/rc.local >/dev/null
if [ $? -eq 0 ]; then
	echo "battery already exists in rc.local. Re-creating."
	# battery already in rc.local, but make sure correct:
	sed -i "s/^.*battery.*$/python \/boot\/battery\/battery.py \&/g" /etc/rc.local >/dev/null
else
